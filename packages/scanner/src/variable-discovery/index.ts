  ...
  for (const item of items) {
    if (!item.rawComment) {
      ...
      if (example?.rawComment) {
        // Merge user-authored documentation from .env.example comments while
        // filtering out lines Pookoo itself generates (Used in:, Declared in:,
        // Auto-generated by Pookoo, section markers). Filtering per line keeps
        // mixed/user-authored comments instead of discarding the whole block.
        const keptLines = example.rawComment
          .split("\n")
          .map((line) => line.replace(/^\s*#\s*/, ""))
          .filter((line) => !GENERATED_MARKER_LINE.test(line));
        const merged = keptLines.join("\n").trim();
        if (merged) {
          item.rawComment = merged;
        }
      }
    }
  }
...
/** Lines Pookoo writes into generated `.env.example` files. */
const GENERATED_MARKER_LINE = /^(Used in:|Declared in:|Auto-generated by Pookoo|→|[─═])/;
